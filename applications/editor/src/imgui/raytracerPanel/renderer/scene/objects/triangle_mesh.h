...
public:
	...
	bool intersect(const ray& p_ray, point3& t, vec3& normal, glm::vec2& uv) const override
	{
		...
		{
			const point3 v0 = triangles.at(i).at(0);
			const point3 v1 = triangles.at(i).at(1);
			const point3 v2 = triangles.at(i).at(2);

			glm::vec2 hit_uv{0, 0};
			if (intersectRayTriangle(acne_corrected_origin, normalize(p_ray.get_direction()), v0, v1, v2, hit_uv,
			                         distance) &&
